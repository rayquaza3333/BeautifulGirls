...
#If none of end game block above was execute. Things go on from here.				
		...
		#give cards to player
			...
			u=cards_left.pop(shuffle(cards_left))
			player.take_card(shuffle(cards_left),total_cards)

			u=cards_left.pop(shuffle(cards_left))
			player.take_card(shuffle(cards_left),total_cards)

			u=cards_left.pop(shuffle(cards_left))
			dealer.take_card(shuffle(cards_left),total_cards)

			u=cards_left.pop(shuffle(cards_left))
			dealer.take_card(shuffle(cards_left),total_cards)

			player.bet(dealer.balance)	
			player.score_cal(turn)
			player.show_card()
			dealer.score_cal(turn)	
			dealer.show_card()
			player.decision()
